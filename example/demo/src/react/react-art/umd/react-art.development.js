...
function appendInitialChild(parentInstance, child) {
  if (typeof child === 'string') {
    ...
    (function () {
      {
        {
          throw ReactError('Text children should already be flattened.');
        }
      }
    })();
    return;
...
function createInstance(type, props, internalInstanceHandle) {
  ...
  (function () {
    if (!instance) {
      {
        throw ReactError('ReactART does not support the type "' + type + '"');
      }
    }
  })();
...
function getChildHostContextForEventComponent() {
  return NO_CONTEXT;
}
...
function insertBefore(parentInstance, child, beforeChild) {
  (function () {
    if (!(child !== beforeChild)) {
      {
        throw ReactError('ReactART: Can not insert node before itself');
      }
    }
  })();
...
function updateDehydratedSuspenseComponent(current, workInProgress, renderExpirationTime) {
  ...
  if (didReceiveUpdate || hasContextChanged$$1) {
    ...
    registerSuspenseInstanceRetry(suspenseInstance, retryTimedOutBoundary$$1.bind(null, current));
    return null;
  } else {
    // This is the first attempt.
...
function beginWork(current, workInProgress, renderExpirationTime) {
  ...
  switch (workInProgress.tag) {
    ...
    case EventComponent:
      {
        if (enableEventAPI) {
          return updateEventComponent(current, workInProgress, renderExpirationTime);
        }
        break;
      }
    case EventTarget:
...
function safelyCallComponentWillUnmount(current, instance) {
  {
    ...
    if (hasCaughtError()) {
      ...
      captureCommitPhaseError$$1(current, unmountError);
    }
  }
}
    ...
    if (typeof ref === 'function') {
      {
        ...
        if (hasCaughtError()) {
          ...
          captureCommitPhaseError$$1(current, refError);
        }
      }
    } else {
      ref.current = null;
...
function safelyCallDestroy(current, destroy) {
  {
    ...
    if (hasCaughtError()) {
      ...
      captureCommitPhaseError$$1(current, error);
    }
  }
}
...
function commitUnmount(current) {
  ...
  switch (current.tag) {
    ...
    case EventComponent:
      {
        if (enableEventAPI) {
          var rootContainerInstance = current.stateNode.rootInstance;
          var responder = current.type.responder;
...
function commitBeforeMutationEffects() {
  while (nextEffect$1 !== null) {
    if ((nextEffect$1.effectTag & Snapshot) !== NoEffect) {
      ...
      var current = nextEffect$1.alternate;
...
function commitLayoutEffects(root, committedExpirationTime) {
  ...
  while (nextEffect$1 !== null) {
    ...
    if (effectTag & (Update | Callback)) {
      recordEffect();
      var current = nextEffect$1.alternate;
      commitLifeCycles(root, current, nextEffect$1, committedExpirationTime);
    }
