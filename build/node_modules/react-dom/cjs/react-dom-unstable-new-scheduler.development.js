 ...
 * react-dom-unstable-new-scheduler.development.js
 *
...
var eventResponderContext = {
  ...
  addRootEventTypes: function (doc, rootEventTypes) {
    ...
    var eventComponent = currentFiber;
...
function createResponderEvent(topLevelType, nativeEvent, nativeEventTarget, eventSystemFlags) {
  return {
...
function processEventQueue() {
  var _currentEventQueue = currentEventQueue,
      ...
      discrete = _currentEventQueue.discrete;
...
function handleTopLevelType(topLevelType, fiber, responderEvent, isRootLevelEvent) {
...
function runResponderEventsInBatch(topLevelType, targetFiber, nativeEvent, nativeEventTarget, eventSystemFlags) {
  if (enableEventAPI) {
    currentEventQueue = createEventQueue();
    var responderEvent = createResponderEvent(topLevelType, nativeEvent, nativeEventTarget, eventSystemFlags);
    ...
    if (rootEventComponents !== undefined) {
      ...
      for (var i = 0; i < rootEventComponentFibers.length; i++) {
        ...
        handleTopLevelType(topLevelType, rootEventComponentFiber, responderEvent, true);
      }
    }
    processEventQueue();
  }
...
function unmountEventResponder(responder, fiber) {
  var onUnmount = responder.onUnmount;
  if (onUnmount !== undefined) {
    var _fiber$stateNode2 = fiber.stateNode,
        props = _fiber$stateNode2.props,
        state = _fiber$stateNode2.state;
...
function handleEventComponent(eventResponder, rootContainerInstance) {
  if (enableEventAPI) {
...
function unmountEventComponent(eventResponder, rootContainerInstance, internalInstanceHandle) {
  if (enableEventAPI) {
  ...
  if (enableEventAPI) {
    if (type === REACT_EVENT_TARGET_TOUCH_HIT) {
      if (true && canUseDOM) {
        ...
        !(computedStyles.getPropertyValue('zIndex') !== '') ? warning$1(false, '<TouchHitTarget> inserts an empty <div> with "z-index" of "-1". ' + 'This requires its parent DOM node to have a "z-index" great than "-1",' + 'but the parent DOM node was found to no "z-index" value set.' + ' Try using a "z-index" value of "0" or greater.') : void 0;
      }
    }
  }
}
...
var resumeTimers = function () {
  ...
  if (currentFiber$1 !== null) {
    resumeTimersRecursively(currentFiber$1);
  }
};
...
var shouldYield$2 = Scheduler.unstable_shouldYield;
...
var unstableNewScheduler = ReactDOM$3.default || ReactDOM$3;

module.exports = unstableNewScheduler;
  })();
}
